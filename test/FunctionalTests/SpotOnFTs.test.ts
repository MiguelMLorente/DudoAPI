...
import * as mockSpotOnAction from "../Mocks/ MockActions/SpotOnActionMock"
...
@suite class SpotOnFunctionalTests {
    ...
    @test 'Starting player cannot spot on since there is not an existing bid'() {
        _chai.expect( () => {
            this.response = handleRequest(this.getCorrectSpotOnAction(), this.realServerData);
        }).to.not.throw();
        _chai.expect(this.response.channel).to.be.eq(ResponseChannel.ERROR);
        _chai.expect(this.response.data[0].sentData).to.be.eq(ErrorMessage.SPOT_NO_BID);
    }
    ...
    @test 'Incorrect format, missing game ID' () {
        handleRequest(this.getCorrectBidAction(1,3), this.realServerData);
        _chai.expect( () => {
            this.response = handleRequest(this.getIncorrectSpotOnAction(), this.realServerData);
        }).to.not.throw();
        _chai.expect(this.response.channel).to.be.eq(ResponseChannel.ERROR);
        _chai.expect(this.response.data.length).to.be.eq(1);
